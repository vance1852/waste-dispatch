...
func (s *TaskService) RecoverStale(ctx context.Context, staleCutoff time.Time) (int, error) {
	// A recovery sweep must not be torn apart by a shutdown, so it runs to the end
	// on a detached context once it has been started.
	sweepCtx := context.WithoutCancel(ctx)

	tasks, err := s.tasks.ListStaleInProgress(sweepCtx, staleCutoff)
	if err != nil {
		return 0, err
	}
	...
	for _, t := range tasks {
		...
		if err := s.tasks.UpdateWithVersion(sweepCtx, t); err != nil {
