...
type TaskService struct {
	...
	settlement repository.ShiftSettlementRepository
	log        zerolog.Logger
}
...
// WithShiftSettlement attaches the settlement repository used to close a shift.
func (s *TaskService) WithShiftSettlement(settlement repository.ShiftSettlementRepository) *TaskService {
	s.settlement = settlement
	return s
}

// SettleShift closes every listed collection task and applies the collected
// weight to the matching collection point.
func (s *TaskService) SettleShift(ctx context.Context, taskIDs []string, collectedKg map[string]float64) error {
	entries := make([]repository.ShiftSettlementEntry, 0, len(taskIDs))
	for _, id := range taskIDs {
		task, err := s.tasks.GetByID(ctx, id)
		if err != nil {
			return fmt.Errorf("load task %s: %w", id, err)
		}
		weight := collectedKg[id]
		if err := task.Complete(weight); err != nil {
			return fmt.Errorf("complete task %s: %w", id, err)
		}
		point, err := s.points.GetByID(ctx, task.PointID)
		if err != nil {
			return fmt.Errorf("load point %s: %w", task.PointID, err)
		}
		newLoad := point.CurrentLoadKg - weight
		if newLoad < 0 {
			newLoad = 0
		}
		point.CurrentLoadKg = newLoad
		point.UpdateStatus()
		entries = append(entries, repository.ShiftSettlementEntry{Task: task, Point: point})
	}

	if err := s.settlement.SettleShift(ctx, entries); err != nil {
		return fmt.Errorf("settle shift: %w", err)
	}

	s.log.Info().Int("tasks", len(entries)).Msg("collection shift settled")
	return nil
}
