...
func TestCreditService_Redeem_WithinBalanceSucceeds(t *testing.T) {
	db := openServiceTestDB(t)
	repo := reposqlite.NewCreditRepository(db)
	svc := service.NewCreditService(repo, zerolog.Nop())
	ctx := context.Background()
	...
	_, _ = svc.EarnCredit(ctx, service.EarnCreditRequest{
		ResidentID: "resident-within-001", Amount: 80,
		IdempotencyKey: uuid.New().String(), CreatedBy: "system",
	})

	tx, err := svc.RedeemCredit(ctx, service.RedeemCreditRequest{
		ResidentID: "resident-within-001", Amount: 50,
		IdempotencyKey: uuid.New().String(), CreatedBy: "system",
	})
	if err != nil {
		t.Fatalf("RedeemCredit within balance error: %v", err)
	}
	if tx.BalanceAfter != 30 {
		t.Errorf("balance_after = %d, want 30", tx.BalanceAfter)
	}

	credit, err := svc.GetBalance(ctx, "resident-within-001")
	if err != nil {
		t.Fatalf("GetBalance error: %v", err)
	}
	if credit.Balance != 30 {
		t.Errorf("balance = %d, want 30", credit.Balance)
	}
}
