...
// RetireVehicle takes a vehicle out of service permanently.
func (s *VehicleService) RetireVehicle(ctx context.Context, vehicleID string) (*domain.Vehicle, error) {
	v, err := s.vehicles.GetByID(ctx, vehicleID)
	if err != nil {
		return nil, err
	}

	if err := v.Retire(); err != nil {
		return nil, err
	}

	if err := s.vehicles.UpdateWithVersion(ctx, v); err != nil {
		return nil, fmt.Errorf("retire vehicle: %w", err)
	}

	s.log.Info().Str("vehicle_id", vehicleID).Msg("vehicle retired")
	return v, nil
}
