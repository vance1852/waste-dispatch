...
func TestVehicleService_AssignDriver_Concurrent_OnlyOneWins(t *testing.T) {
	db := openServiceTestDB(t)
	repo := reposqlite.NewVehicleRepository(db)
	svc := service.NewVehicleService(repo, zerolog.Nop())
	ctx := context.Background()
	...
	for i := 0; i < goroutines; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			driverID := "driver-" + string(rune('A'+idx))
			_, err := svc.AssignDriver(ctx, v.ID, driverID)
			errChan <- err
		}(i)
	}
	wg.Wait()
	...
	// At most one should succeed; others fail with version conflict or transition error.
	if successCount > 1 {
		t.Errorf("concurrent assign: %d goroutines succeeded, expected <= 1", successCount)
	}
}
