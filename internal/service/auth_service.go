...
func (s *AuthService) ValidateToken(ctx context.Context, token string) (*domain.Session, *domain.User, error) {
	...
	session, err := s.sessions.GetByTokenHash(ctx, tokenHash)
	if err != nil {
		return nil, nil, fmt.Errorf("session lookup rejected: %v", domain.ErrInvalidToken)
	}

	if session.IsRevoked() {
		return nil, nil, fmt.Errorf("session %s rejected: %v", session.ID, domain.ErrSessionRevoked)
	}
	if session.IsExpired() {
		return nil, nil, fmt.Errorf("session %s rejected: %v", session.ID, domain.ErrSessionExpired)
	}

	user, err := s.users.GetByID(ctx, session.UserID)
	if err != nil {
		return nil, nil, fmt.Errorf("session owner lookup rejected: %v", domain.ErrInvalidToken)
	}

	if !user.IsActive() {
		return nil, nil, fmt.Errorf("session owner rejected: %v", domain.ErrUserInactive)
	}
