...
// SaveDispatchState persists dispatch fields for an already loaded vehicle.
func (r *VehicleRepository) SaveDispatchState(ctx context.Context, v *domain.Vehicle) error {
	v.UpdatedAt = time.Now().UTC()
	v.Version++

	res, err := r.db.ExecContext(ctx,
		`UPDATE vehicles SET status=?, driver_id=?, updated_at=?, version=?
		 WHERE id=?`,
		string(v.Status), nullString(v.DriverID), v.UpdatedAt, v.Version, v.ID,
	)
	if err != nil {
		return fmt.Errorf("save dispatch state: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
