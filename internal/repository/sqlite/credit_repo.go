...
// ApplyDelta moves a resident balance by delta in a single statement and returns
// the balance after the change.
func (r *CreditRepository) ApplyDelta(ctx context.Context, residentID string, delta int64) (int64, error) {
	res, err := r.db.ExecContext(ctx,
		`UPDATE resident_credits
		 SET balance = balance + ?, updated_at = ?, version = version + 1
		 WHERE resident_id = ?`,
		delta, time.Now().UTC(), residentID,
	)
	if err != nil {
		return 0, fmt.Errorf("apply credit delta: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return 0, domain.ErrCreditNotFound
	}

	var balance int64
	if err := r.db.QueryRowContext(ctx,
		`SELECT balance FROM resident_credits WHERE resident_id = ?`, residentID,
	).Scan(&balance); err != nil {
		return 0, fmt.Errorf("read balance after delta: %w", err)
	}
	return balance, nil
}
