...
// ShiftSettlementRepository closes a whole collection shift in one unit of work.
type ShiftSettlementRepository interface {
	// SettleShift persists the completion of every task in a shift together with
	// the resulting collection point loads.
	SettleShift(ctx context.Context, entries []ShiftSettlementEntry) error
}

// ShiftSettlementEntry is one task/point pair inside a shift settlement.
type ShiftSettlementEntry struct {
